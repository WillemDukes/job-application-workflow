...
1. **Verify CODEOWNERS is yours** -- `.github/CODEOWNERS` names `@WillemDukes`; forks should replace it with their own handle.
...
- **`pre-commit-secrets.sh.template`** -- Pre-commit hook that blocks commits containing API keys, key files, credentials, and custom forbidden tokens.
- **`forbidden-tokens.txt.template`** -- Customizable blocklist for environment-specific strings (hostnames, paths, key prefixes).
- **`setup-hooks.sh`** -- Safe installer that chains with existing hooks and backs up to `~/.config/repo-template/hooks/` (path kept from the upstream template).
